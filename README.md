...
- 如何把跑步、力量训练和训练计划抽象到可扩展的 Room 数据模型。
- 为什么用 Repository 隔离 UI 与本地数据库，并用 ViewModel 管理页面状态。
- 如何用 Compose Canvas 实现轻量图表，避免引入重型图表依赖。
- 如何设计 JSON 备份恢复流程，兼顾用户数据迁移和 Room 外键关系。
- 如何通过 schema 文件和显式 migration 管理本地数据库演进。
